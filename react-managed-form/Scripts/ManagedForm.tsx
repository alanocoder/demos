...
export class ManagedForm extends React.Component<Props, IState> {
    constructor(props: Props) {
        super(props);
        const { defaultValues, model, onChange } = props;
        ...
        if (defaultValues && model) {
            for (var name in defaultValues) { // mark all fields that have defaultValue as touched. so that any error will be shown right away
                if (defaultValues[name] && model[name]) touched[name] = true;
            }
        }
        var errors = this.validate(values); // initialize errors (null if no errors)
        this.state = { values, errors, touched };
        if (onChange) onChange(this, null); // first call, no matter what (allows the higher component obtain a reference to ManagedForm)
    }
    ...
    onBlur = (evt: any) => {
        ...
        if ((!evt.target.tagName || this.state.touched[name]) && this.state.touched[name] !== 'touched') { // no tagName if it's wrapping some other ui component like react-md SelectField
            ...
            this.setState({ touched },
                this.props.onChange ? () => this.props.onChange(this, name) : null); // onChanged is invoked also when touched changes (touched errors only show up when touched changes)
        }
    }
